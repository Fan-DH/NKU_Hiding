...
% 提取秘密图像
stego_img_read = double(imread('stego_modify_img.png'));
ext_img_bits = extract_modify(stego_img_read, img_len, Q);

% 计算 BER 并保存提取的图像
ber_img = sum(ext_img_bits ~= img_bits) / img_len;
fprintf('秘密图像提取完成，BER: %.4f\n\n', ber_img);
ext_img = logical(reshape(ext_img_bits, size(secret_img_bin)));
imwrite(ext_img, 'extracted_secret_modify_img.png');

% 加噪并再次提取
fprintf('--- 加噪测试（高斯噪声） ---\n');
noisy_stego_img_uint8 = imnoise(stego_img_uint8, 'gaussian', 0, 0.005);
imwrite(noisy_stego_img_uint8, 'noisy_stego_modify_img.png');
noisy_stego_img_read = double(imread('noisy_stego_modify_img.png'));
noisy_ext_img_bits = extract_modify(noisy_stego_img_read, img_len, Q);
...
imwrite(noisy_ext_img, 'noisy_extracted_secret_modify_img.png');


%% 4. 实验二：隐藏和测试文本信息
fprintf('====== 实验二：隐藏和测试文本信息 ======\n');
stego_text = embed_modify(cover, text_bits, Q);

% 保存含密图像
stego_text = max(min(round(stego_text), 255), 0);
stego_text_uint8 = uint8(stego_text);
imwrite(stego_text_uint8, 'stego_modify_text.png');
...
% 提取文本信息
stego_text_read = double(imread('stego_modify_text.png'));
ext_text_bits = extract_modify(stego_text_read, text_len, Q);
...
imwrite(noisy_stego_text_uint8, 'noisy_stego_modify_text.png');
noisy_stego_text_read = double(imread('noisy_stego_modify_text.png'));
noisy_ext_text_bits = extract_modify(noisy_stego_text_read, text_len, Q);
