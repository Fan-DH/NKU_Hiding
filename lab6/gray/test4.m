...
% 读取尺寸信息
info_path = fullfile(in_dir, 'secret_info.mat');
...
for p = planes
    stego_name = fullfile(in_dir, sprintf('stego_plane%d.png', p));
    fprintf('\n--- 处理隐藏在第 %d 位平面的载体图像 %s ---\n', p, stego_name);
    ...
    for n_idx = 1:3
        ...
        elseif n_idx == 2
            noisy_stego = imnoise(stego_img, 'salt & pepper', 0.01);
        elseif n_idx == 3
            noisy_stego = imnoise(stego_img, 'salt & pepper', 0.05);
        end
        
        % 2. 提取秘密图像
        extr_bit_plane = bitget(noisy_stego, p);
        ...
        % 保存提取出来的图像
        out_name = fullfile(out_dir, sprintf('extr_plane%d_noise%d.png', p, n_idx));
